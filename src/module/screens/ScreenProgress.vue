    ...
    <article v-if="isDryRun" class="card">
      <header class="card-header">
        <div>
          ...
          <p>No data was written. Tweak strategies and re-run, or generate for real.</p>
        </div>
    ...
    <article v-else class="card">
      ...
      <div v-else-if="done" class="banner banner-success">
        ...
        <div>
          ...
          <p>Wrote {{ rowsWritten.toLocaleString() }} rows in {{ elapsedLabel }}.</p>
        </div>
      </div>
...
watch(
  ...
  async (id) => {
    ...
    // Always seed from audit row first — this populates known totals/state
    // even for terminal runs where SSE history has been purged.
    try {
      ...
      // ignore; SSE may still bring fresh state
    }

    // Connect SSE — works for live runs; no-ops for purged ones.
    progress.start(api.progressUrl(id));
  },
  { immediate: true }
);
...
watch(progress.events, (events) => {
  for (const evt of events.slice(rowsWritten.value === 0 ? 0 : -1)) {
    ...
    if (evt.type === 'complete') done.value = true;
    if (evt.type === 'error') {
      done.value = true;
